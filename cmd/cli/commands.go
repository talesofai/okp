...
func cmdEmbed() *cobra.Command {
	var domain string
	var limit int
	var stats bool
	cmd := &cobra.Command{
		Use:   "embed",
		Short: "生成/补全 concept 向量（用于跨语言语义搜索）",
		Long: `调用 embedding API 为 concepts 生成向量，存入 PG。

新写入的 concept 会自动异步触发，此命令用于：
  1. 存量数据 backfill（首次部署后运行一次）
  2. 查看 embedding 状态统计
  3. 重试失败的 concepts`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if stats {
				resp, err := doRequest("GET", "/api/v1/embed/stats", nil)
				if err != nil {
					return fmt.Errorf("请求失败: %w", err)
				}
				var result map[string]any
			...
			fmt.Fprintf(os.Stderr, "开始生成向量 (domain=%s limit=%d)...\n", domain, limit)
			resp, err := doRequest("POST", "/api/v1/embed/batch?"+params.Encode(), nil)
			if err != nil {
				return fmt.Errorf("请求失败: %w", err)
			}
			var result map[string]any
	...
	cmd.Flags().StringVarP(&domain, "domain", "d", "", "只处理指定领域")
	cmd.Flags().IntVarP(&limit, "limit", "n", 500, "本次处理上限")
	cmd.Flags().BoolVar(&stats, "stats", false, "查看 embedding 状态统计")
	return cmd
}
